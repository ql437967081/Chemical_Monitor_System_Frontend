...
class ExpressIn extends React.Component {
    ...
    scanProduct = () => {
        ...
        /*axios({
            method: 'post',
            url: baseUrl + 'input_product/' + expressId
        }).then(function (res) {
        ...
        post(baseUrl + 'input_product/' + expressId, undefined).then(function (res) {
            if (checkTokenExpiration(res, this.props.history))
                return;
            const resp = res.data;
            const changedState = {};
            ...
            <Card title={'物流入库'}>
                { !begin ? (
                    ...
                    <Form layout={"inline"}>
                        ...
                        <Form.Item>
                            { allCompleted ? (
                                <Alert message="本次物流入库完成" type="success" />
                            ) : (
...
export default withRouter(Form.create()(ExpressIn));
